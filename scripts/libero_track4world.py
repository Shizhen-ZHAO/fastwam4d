...
def validate_paths(paths: dict, *, cache_required: bool = False):
    # Frozen-base checkpoint is optional for full Wan+ActionDiT training.
    files = ("dataset_stats", "track4world_checkpoint")
    directories = (
        "model_base",
        "track4world_repo",
        "da3_model",
...
def apply_path_overrides(paths: dict, hydra_args: list[str]) -> dict:
    """Preflight must inspect the same paths Hydra will actually use."""
    if any(arg.startswith(('paths=', '+paths=')) for arg in hydra_args):
        raise ValueError('Select the path config using --paths/GEOMETRY_PATHS, not paths=')
    result = OmegaConf.create({'paths': paths})
    for arg in hydra_args:
        if arg.startswith('paths.'):
            key, value = arg.split('=', 1)
            OmegaConf.update(result, key, OmegaConf.from_dotlist(['value=' + value]).value, merge=False)
    return OmegaConf.to_container(result.paths, resolve=True)


def validate_model_assets(paths: dict, *, evaluation: bool, initial_checkpoint=None):
    """Fail before allocating a multi-billion-parameter model or downloading."""
    base = Path(paths['model_base']).expanduser()
    common = base / 'DiffSynth-Studio/Wan-Series-Converted-Safetensors'
    dit = base / paths['model_id']
    required = [common / 'Wan2.2_VAE.safetensors']
    if not paths['redirect_common_files']:
        required = [dit / 'Wan2.2_VAE.pth']
    if evaluation:
        text = common / 'models_t5_umt5-xxl-enc-bf16.safetensors' if paths['redirect_common_files'] else dit / 'models_t5_umt5-xxl-enc-bf16.pth'
        required += [text,
                     base / paths.get('tokenizer_model_id', 'Wan-AI/Wan2.1-T2V-1.3B') / 'google/umt5-xxl/tokenizer_config.json']
    else:
        required.append(Path(paths['action_dit_checkpoint']).expanduser())
        if not list(dit.glob('diffusion_pytorch_model*.safetensors')):
            raise FileNotFoundError(f'Missing Wan DiT weights under {dit}')
        index = dit / 'diffusion_pytorch_model.safetensors.index.json'
        if index.is_file():
            required.extend(dit / shard for shard in set(json.loads(index.read_text())['weight_map'].values()))
    if initial_checkpoint:
        required.append(Path(initial_checkpoint).expanduser())
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError('Missing local model assets: ' + ', '.join(missing))
