#!/usr/bin/env bash
set -euo pipefail
# Within each variant, GEOMETRY_ENABLED is the ablation switch.
...
export FASTWAM_VARIANT="${FASTWAM_VARIANT:-uncond}"
case "$FASTWAM_VARIANT" in
  uncond) default_batch=4; default_accum=2; output_variant='' ;;
  joint) default_batch=8; default_accum=1; output_variant='joint/' ;;
  *) echo 'FASTWAM_VARIANT must be uncond or joint' >&2; exit 2 ;;
esac
...
if [[ -z "${OUTPUT_DIR:-}" ]]; then OUTPUT_DIR="\${paths.output_root}/${output_variant}geometry_${GEOMETRY_ENABLED}/${RUN_ID}"; fi
# Same bf16, ZeRO-1 and per-variant B/GAS as existing baseline launchers.
command=(accelerate launch --config_file scripts/accelerate_configs/accelerate_zero1_ds.yaml
  ...
  scripts/geometry_ablation.py train --variant "$FASTWAM_VARIANT" --paths "$GEOMETRY_PATHS"
  "geometry_enabled=${GEOMETRY_ENABLED}" "batch_size=${BATCH_SIZE:-$default_batch}" "gradient_accumulation_steps=${GRAD_ACCUM:-$default_accum}"
